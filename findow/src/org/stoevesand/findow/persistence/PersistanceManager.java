...
public class PersistanceManager {
    ...
    public List<FinTransaction> getTx(FinUser user, Long accountId, int days) throws FinErrorHandler {
        ...
        if (accountId != null) {
            ...
            if (account != null) {
                List<FinTransaction> subResult = em.createQuery(
                        "select t from Transaction t where t.accountId=:aid and t.bookingDate > current_date - :daydelta order by t.bookingDate desc",
                        FinTransaction.class).setParameter("daydelta", days).setParameter("aid", account.getId()).getResultList();
            ...
            if (accounts != null) {
                for (FinAccount account : accounts) {
                    List<FinTransaction> subResult = em
                            .createQuery("select t from Transaction t where t.accountId=:aid and t.bookingDate > current_date - :daydelta",
                                    FinTransaction.class)
                            .setParameter("daydelta", days).setParameter("aid", account.getId()).getResultList();
                    result.addAll(subResult);
                }
            }
        }
    ...
    public FinUser getUserByName(String id) {
        ...
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
        List<FinUser> result = entityManager.createQuery("select t from User t where t.name = :username", FinUser.class).setParameter("username", id)
                .getResultList();
        if (result.size() > 0) {
    ...
    public FinUser getUserByExternalName(String id) {
        ...
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
        List<FinUser> result = entityManager.createQuery("select t from User t where t.backendName = :username", FinUser.class).setParameter("username", id)
                .getResultList();
        if (result.size() > 0) {
            ret = (FinUser)result.get(0);
        }
    ...
    public FinTransaction getTxByExternalId(String sourceId) {
        ...
        List<FinTransaction> result = entityManager.createQuery("select t from Transaction t where t.sourceId=:sourceid").setParameter("sourceid", sourceId)
                .getResultList();
        if (result.size() > 0) {
    ...
    public List<FinCategorySum> getCategorySummary(FinUser user, Long accountId) {
        ...
        Query q = entityManager.createNativeQuery(
                "select coalesce(category_id,0) as category_id, count(*) as count, sum(amount) as sum from transactions t where t.account_id=:aid group by t.category_id",
                FinCategorySum.class).setParameter("aid", account.getSourceId());
    ...
    public List<FinAccount> getAccounts(FinUser user) throws FinErrorHandler {
        ...
        List<FinAccount> accounts = entityManager.createQuery("select a from Account a where a.user=:user", FinAccount.class).setParameter("user", user)
                .getResultList();
    ...
    public FinAccount getAccountByExternalId(FinUser user, String accountId) {
        ...
        List<FinAccount> result = entityManager.createQuery("select t from Account t where t.sourceId = :sid", FinAccount.class)
                .setParameter("sid", accountId).getResultList();
    ...
    public void deleteAccounts(FinUser user, int connectionId) {
        ...
        List<FinAccount> accounts = entityManager.createQuery("select a from Account a where a.bankConnectionId=:bcid", FinAccount.class)
                .setParameter("bcid", connectionId).getResultList();
    ...
    public void checkAccount(FinUser user, FinAccount account) {
        ...
        List<FinAccount> accounts = entityManager.createQuery("select a from Account a where a.sourceId=:sid", FinAccount.class)
                .setParameter("sid", account.getSourceId()).getResultList();
    ...
    public void checkAccounts(FinUser user, List<FinAccount> accounts) {
        ...
        for (FinAccount account : accounts) {
            ...
            List<FinAccount> accs = entityManager.createQuery("select a from Account a where a.sourceId=:sid", FinAccount.class)
                    .setParameter("sid", account.getSourceId()).getResultList();
    ...
    public List<FinBank> searchBanks(String search) {
        ...
        List<FinBank> accounts = entityManager.createQuery("select a from Bank a where UPPER(a.name) like :search", FinBank.class)
    ...
    public FinBank getBankByCode(String bankCode) {
        ...
        List<FinBank> result = entityManager.createQuery("select t from Bank t where t.blz = :bc", FinBank.class).setParameter("bc", bankCode)
                .getResultList();
